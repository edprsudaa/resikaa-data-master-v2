...
class TbPegawai extends \yii\db\ActiveRecord
{
    ...
    public static function getPegawai($id)
    {
        ...
        SELECT 
            a.*, 
            (select b.id from " . RiwayatPendidikan::tableName() . " as b where b.nip=a.id_nip_nrp order by tanggal_sttb desc limit 1) as rwyt_pnddkn,
            (select c.id from " . RiwayatKepangkatan::tableName() . " as c where c.nip=a.id_nip_nrp order by sk_tanggal_pangkat desc limit 1) as rwyt_pngkt,
            (select d.id from " . RiwayatJabatan::tableName() . " as d where d.nip=a.id_nip_nrp order by sk_pelantikan_tanggal desc limit 1) as rwyt_jbtn,
            (select e.id from " . RiwayatPenempatan::tableName() . " as e where e.id_nip_nrp=a.id_nip_nrp order by tanggal desc limit 1) as rwyt_pnmptn
        from " . TbPegawai::tableName() . " as a 
        where a.id_nip_nrp='$id' limit 1")->queryOne();
        return $data;
    }

    public static function find()
    {
        return new TbPegawaiQuery(get_called_class());
    }

    public function getRiwayatPenempatan()
    {
        return $this->hasMany(RiwayatPenempatan::className(),['id_nip_nrp'=>'id_nip_nrp']);
    }
    public function getPltPlh()
    {
        return $this->hasMany(TbUnitPltPlh::className(),['id_nip_nrp'=>'id_nip_nrp']);
    }
}
