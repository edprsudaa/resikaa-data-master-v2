 ...
 * @property string|null $perubahan_nip
 */
class PegawaiPegawai extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    ...
    public function rules()
    {
        return [
            ...
            // [[ 'no_telepon_1', 'no_telepon_2'], 'string', 'max' => 15],
            // [['alamat_tempat_tinggal', 'tanda_tangan'], 'string', 'max' => 100],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'jenis_kepegawaian_id' => 'Jenis Kepegawaian',
            'npwp' => 'Npwp',
            'nomor_ktp' => 'Nomor Ktp',
            ...
            'tipe_user' => 'Tipe User',
            'kode_dokter_maping_simrs' => 'Kode Simrs',
            'niptk' => 'Niptk',
            'tanda_tangan' => 'TTD'
            // 'perubahan_nip' => 'Perubahan Nip',
        ];
    }
    ...
    public static function getDokter()
    {
        ...
        with perawat_bidan AS (SELECT a.id_nip_nrp, a.nama_lengkap, a.gelar_sarjana_depan, a.gelar_sarjana_belakang, 
        (select b.kode_jabatan from ".RiwayatJabatan::tableName()." b where b.nip=a.id_nip_nrp order by sk_pelantikan_tanggal desc limit 1) as jabatan, 
        (select c.id from ".RiwayatPenempatan::tableName()." c where c.id_nip_nrp=a.id_nip_nrp order by tanggal desc limit 1) as rwyt_penempatan,
        (select d.id from ".RiwayatStr::tableName()." d where d.id_nip_nrp=a.id_nip_nrp order by tanggal_berlaku desc limit 1) as str,
        (select e.id from ".RiwayatSipp::tableName()." e where e.id_nip_nrp=a.id_nip_nrp order by tanggal_berlaku desc limit 1) as sipp,
        (select f.id from ".RiwayatSpklinis::tableName()." f where f.id_nip_nrp=a.id_nip_nrp order by tanggal_berlaku desc limit 1) as spk,
        (select g.sdm_rumpun from ".RiwayatPenempatan::tableName()." g where g.id_nip_nrp=a.id_nip_nrp order by g.tanggal desc limit 1) as sdm_rumpun,
        (select h.tanggal_berlaku from ".RiwayatStr::tableName()." h where h.id_nip_nrp=a.id_nip_nrp order by h.tanggal_berlaku desc limit 1) as tgl_str,
        (select i.tanggal_berlaku from ".RiwayatSipp::tableName()." i where i.id_nip_nrp=a.id_nip_nrp order by i.tanggal_berlaku desc limit 1) as tgl_sip,
        (select j.tanggal_berlaku from ".RiwayatSpklinis::tableName()." j where j.id_nip_nrp=a.id_nip_nrp order by j.tanggal_berlaku desc limit 1) as tgl_spk
        FROM ".PegawaiPegawai::tableName()." a)
        
        select * from perawat_bidan where (sdm_rumpun=1) and (
            (tgl_str < '2021-02-15' or tgl_str is null) OR (tgl_sip < '2021-02-15' or tgl_sip is null) OR (tgl_spk < '2021-02-15' or tgl_spk is null))")->queryAll();
    ...
    {
        return $this->hasOne(PegawaiStatusKepegawaian::className(),['id'=>'status_kepegawaian_id']);
    }
    public function getJenisKepegawaian()
    {
        return $this->hasOne(PegawaiJenisPns::className(),['id'=>'jenis_kepegawaian_id']);
    }

    public function getAkunUser()
    {
        return $this->hasOne(AkunAknUser::className(),['id_pegawai'=>'pegawai_id']);
    }
}
